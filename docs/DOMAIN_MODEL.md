...
| `turn_id` | string | Runtime event payloads for model/tool turns | Event correlation. **ISSUE**: Some producers still call this `call_id` — see §13 |
...
Appears identically in: `trial_input_v1`, `trial_output_v1`, `event_envelope_v1`, `benchmark_prediction_record_v1`, `benchmark_score_record_v1`
...
### Runtime Command
The user-provided program that executes a trial. Consumes `trial_input.json`, produces `result.json` (`agent_result_v1`). Optional richer integrations can emit JSONL events/traces.
...
- Appears in: `event_envelope_v1.redaction.mode`, `trace_manifest_v1.redaction.mode`, `harness_manifest_v1.redaction.content_policy`
...
| Control Actions | `continue`, `stop`, `checkpoint` | Runtime event payloads and control acknowledgements |
...
```ts
ok: true
command: 'knobs-validate' | 'schema-validate'  # ISSUE: knobs → params
valid: true
[key: string]: unknown
```
...
Methods: `describe()`, `run()`, `runDev()`, `runExperiment()`, `publish()`, `validateKnobs()`, `validateSchema()`
...
```
Appears in: `event_envelope_v1.outcome` and runtime event payloads.

### The `redaction` block (3 variants — **ISSUE**: should be 1)
1. **event_envelope_v1**: `{ applied: bool, mode: "store"|"hash"|"drop" }`
2. **harness_manifest_v1**: `{ mode: "on"|"off", applied_by_harness: bool, content_policy: "store"|"hash"|"drop" }` — **ISSUE**: `mode` overloaded
...
```
Appears in: `trial_output_v1.error`, `benchmark_score_record_v1.error`, and runtime event payloads.
...
## 10. Runtime Event Payload Types

Common JSONL event payload shapes:
