...
void ExtensionHelper::LoadExternalExtension(DatabaseInstance &db, FileSystem &fs, const string &extension) {
	...
	auto extension_name = GetExtensionName(extension);
	if (!IsFullPath(extension)) {
		if (extension_name == "json") {
#if DUCKDB_EXTENSION_JSON_LINKED
			DuckDB duckdb(db);
			duckdb.LoadExtension<JsonExtension>();
			return;
#endif
		}
		if (extension_name == "sqlite_scanner") {
#if DUCKDB_EXTENSION_SQLITE_SCANNER_LINKED
			DuckDB duckdb(db);
			duckdb.LoadExtension<SqliteScannerExtension>();
			return;
#endif
		}
	}
