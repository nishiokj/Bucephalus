...
# When a fallback manifest is used, the merged manifest is written to a separate
# file so the primary manifest retains accurate provenance for images actually
# built in this run.  The merged manifest is what gets verified downstream.
MERGED_MANIFEST="${MANIFEST}"
if [[ -n "${FALLBACK_MANIFEST}" && -f "${FALLBACK_MANIFEST}" ]]; then
  MERGED_MANIFEST="${WORK_DIR}/merged-cloud-image-build-manifest.json"
fi
...
// Load and merge fallback manifest if specified.  Inherited images are tagged
// with a provenance_source field so downstream consumers can distinguish images
// built in this run from those reused from a prior verified release.
  ...
  for (const fallbackImg of fallback.images) {
    if (!existingComponents.has(fallbackImg.component)) {
      manifest.images.push({
        ...
        provenance_source: {
          source: "inherited",
          release_version: fallback.release.version,
        },
      });
    }
  }
  // Write the merged manifest to the merged path, not the primary manifest
  await Bun.write(process.env.MERGED_MANIFEST, JSON.stringify(manifest, null, 2));
}
...
const inherited = manifest.images.filter((img) => img.provenance_source?.source === "inherited");
const inheritedSummary = inherited.length > 0
  ? ` (inherited: ${inherited.map((img) => `${img.component}@${img.provenance_source.release_version}`).join(", ")})`
  : "";
...
console.log(`tfvars=${process.env.OUT_PATH}${inheritedSummary}`);
JS
MANIFEST="${MANIFEST}" MERGED_MANIFEST="${MERGED_MANIFEST}" OUT_PATH="${OUT_PATH}" FALLBACK_MANIFEST="${FALLBACK_MANIFEST:-}" bun "${WRITE_JS}"
"${ROOT_DIR}/scripts/release/verify-gcp-image-tfvars.sh" \
  --image-manifest "${MERGED_MANIFEST}" \
  --tfvars "${OUT_PATH}"
