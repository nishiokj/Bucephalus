...
OAUTH_CLI_CLIENT_ID=""
OAUTH_CLI_CLIENT_SECRET_VERSION=""
...
Usage: scripts/deploy/write-gcp-deploy-tfvars.sh --out <path> \
  ...
  --oauth-cli-client-secret-version <number> \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --oauth-cli-client-id)
      ...
      shift 2
      ;;
    --oauth-cli-client-secret-version)
      OAUTH_CLI_CLIENT_SECRET_VERSION="${2:-}"
      shift 2
      ;;
    --oauth-cli-scope)
      OAUTH_CLI_SCOPE="${2:-}"
...
OAUTH_CLI_CLIENT_SECRET_VERSION="${OAUTH_CLI_CLIENT_SECRET_VERSION}" \
...
const values = {
  ...
  oauth_cli_client_secret_secret_version: optional(process.env.OAUTH_CLI_CLIENT_SECRET_VERSION),
...
const serviceChecks = [
  ...
  ["oauth_cli_client_secret_secret_version", /^[1-9][0-9]*$/],
...
const order = [
  ...
  "oauth_user_client_id",
  "oauth_cli_client_id",
  "oauth_cli_client_secret_secret_version",
  "oauth_cli_scope",
  "oauth_jwks_url",
