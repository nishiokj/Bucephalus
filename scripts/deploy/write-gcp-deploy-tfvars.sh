...
DEPLOY_CONTROL_PLANE_SERVICES="true"
...
if (!["true", "false"].includes(values.deploy_control_plane_services)) {
  fail("deploy_control_plane_services must be true or false");
}
const deployServices = values.deploy_control_plane_services === "true";
const checks = deployServices ? [...alwaysChecks, ...serviceChecks] : alwaysChecks;
...
if (deployServices && values.pool_controller_runner_pool_id === "00000000-0000-0000-0000-000000000000") {
...
const lines = [
  ...
  "",
  ...order.map((name) => {
    if (name === "deploy_control_plane_services") {
