...
- **WITH-EVENT run done — and the positives FAIL (critical).** Full arm (events present, 15/18 trials before a teardown flake): the 5 traps correctly stayed **silent**, but castor, brand, and gpu produced **0 alerts even with their events**; customer_of_customer alerted but also alerts state-only (the leak with an event label). **Zero genuinely event-load-bearing positives.** Root cause (agent's own words): it searches the company data for the event's terms ("Bahia", "cicada", "cable"), finds **no link**, and dismisses — because the events are (correctly) region/phenomenon-only and the **data encodes no bridge** (no origin/region/crop attributes; the castor supplier is even listed US). New invariant needed: **EVENT-COMPANY-BRIDGE** — the event→company link must be findable in the data via a shared attribute (origin/region/crop-type/sensitivity), so a search on the event's neutral terms reaches the target entity *without the event naming it*. Until this exists, the state-only "no leak" result is hollow: the link was never expressible either way.
- **Open:** build EVENT-COMPANY-BRIDGE attributes into the materials/commodities and re-verify the full arm fires the positives. POSITIVE-DIVERSITY (only after positives actually work). Intermittent runtime flakes under disk pressure (docker 409 teardown kill, occasional empty response).
