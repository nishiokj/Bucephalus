...
The intuition for these primitives lies in categorizing the resources you declare when running an experiment. Is the lifecycle of this resource NOT owned within the experiment itself? It's an external. Think a persistent database or third party API, we do not materialize these nor take them down. Is Bucephalus responsible for transporting data into and out of this? It's a stage. Think your agent application, or a grader script. The agent app needs to be passed a case in order to handle it, and the grader script needs the result of the agent application in order to run. An Ephemeral is then something that's lifecycle is contained within an experiment, but we we do not have to manually transport data in and out of, think an MCP server, sidecar or mocked temporary database. 
